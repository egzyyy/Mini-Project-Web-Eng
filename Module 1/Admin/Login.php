...
if (isset($_POST['uname']) && isset($_POST['password'])) {
	...
	if (empty($uname)) {
		header("Location: landing.php?error=User Name is required");
	    exit();
	}else if(empty($pass)){
        header("Location: landing.php?error=Password is required");
	    exit();
	}else{
		...
		if (mysqli_num_rows($result) === 1) {
            ...
            if ($row['U_Username'] === $uname && $row['U_Password'] === $pass) {
				...
				header("Location: landing.php?error=Incorect User name or password");
		        exit();
			}
		}else{
			header("Location: landing.php?error=Incorect User name or password");
